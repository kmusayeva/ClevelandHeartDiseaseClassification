---
title: "Heart disease analysis in R"
...
This RMarkdown file contains contains data exploration, data visualization, statistical/epidemiological and predictive modeling analyses of heart disease dataset. The final report was completed on `r date()`.
...
Noninvasive test data are: *exercise electrocardiogram*, exercisethallium scintigraphy and *fluoroscopy for coronary calcium*.
...
We will rename the columns to render to match more or less their description.
...
pvals <- dat %>%
  ...
  summarise(p_value = shapiro.test(max_heart_rate)$p.value) %>%
  mutate(
    label = paste0("Heart disease present=", heart_disease_present, "\n(p = ", round(p_value, 3), ")")
  )

dat_labeled <- dat %>% select(c("max_heart_rate", "heart_disease_present")) %>% left_join(pvals, by = "heart_disease_present")

ggplot(dat_labeled, aes(sample = max_heart_rate)) +
  stat_qq() +
  stat_qq_line() +
  facet_wrap(~ label)+
  labs(
    ...
    y = "Sample Quantiles",
    title = "Q-Q plots of max heart rate by heart disease"
  ) +
...
t.test(max_heart_rate ~ factor(heart_disease_present, levels=c(1, 0)), data = dat, var.equal = FALSE)
```
...
In this section, we will do odds ratio (OR) analysis of association between variables.
...
Regarding the **gender** of individuals, the odds of heart disease in males is $3.5$ times that in females.
...
#### **(Unadjusted) Odds Ratio with Multinomial Variables** {.tabset}
...
```{r echo=FALSE, incorrect-preds, fig.height=5, fig.width=15, warning=FALSE}
